					...
					<table>
						...
						<c:forEach var="r" items="${requestList}">
							<tr>
								...
								<td>
									<c:out value="${r.categoryItem}" />
								</td>
								<td><a href="/fixer/requests/${r.repairNo}">
										<c:out value="${r.receiptTitle}" />
									</a></td>
								<td>
								...
								<td>${r.visitAtText}</td>
								<td>
									<c:out value="${r.receiptAddress}" />
								</td>
								<td>${r.photoCount}</td>
								<td>
									<c:choose>
										<c:when test="${not empty r.myEstimateId}"><span class="done">견적 제출함</span>
										</c:when>
