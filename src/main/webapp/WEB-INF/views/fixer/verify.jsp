				...
				<c:otherwise>
					...
					<c:if test="${profile.fixerApproval eq 'REJECTED'}">
						<p style="color:red;">
							이전 신청이 거절되었습니다.<br>
							사유:
							<c:out value="${profile.fixerRejectReason}" /><br>
							내용을 보완해서 다시 신청해주세요.
						</p>
					</c:if>
					...
					<form action="/fixer/verify" method="post" enctype="multipart/form-data">
						...
						<p>
							...
							<textarea name="fixerIntro" rows="4" cols="50"
								maxlength="500"><c:out value="${profile.fixerIntro}"/></textarea>
						</p>
						<p>
							경력 (200자 이내)<br>
							<input type="text" name="fixerCareer" size="50" maxlength="200"
								value="<c:out value='${profile.fixerCareer}'/>">
						</p>
